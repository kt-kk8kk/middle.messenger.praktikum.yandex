...
### 5. Функциональность и инструменты
Настроен роутинг страниц. Внедрены HTTP API чатов, авторизации и пользователей. Подключен WebSocket для работы с real-time сообщениями.

### 6. Проверка кодстайла и тайпингов
Для проверки стиля кода и тайпингов используйте команду:
```
npm run lint
```

### 7. Тестирование
Проект использует Mocha и Chai для юнит-тестирования.
...
### 8. Тестирование через прекоммит
В проекте настроены прекоммит хуки, которые автоматически выполняют линтинг и юнит-тесты перед каждым коммитом с помощью Husky. Если тесты или линтинг не пройдены, коммит не будет выполнен, что помогает предотвратить попадание некорректного кода в репозиторий.
